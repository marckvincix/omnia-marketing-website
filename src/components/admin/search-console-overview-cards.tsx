import { ArrowDown, ArrowUp, Minus } from "lucide-react";
import type { SearchConsoleOverview, SearchConsoleMetricComparison } from "@/lib/seo/search-console";

function ChangeBadge({ changePercent, invert = false }: { changePercent: number | null; invert?: boolean }) {
  if (changePercent === null) {
    return <span className="text-xs text-muted-foreground">—</span>;
  }
  const rounded = Math.round(changePercent);
  if (rounded === 0) {
    return (
      <span className="inline-flex items-center gap-0.5 text-xs text-muted-foreground">
        <Minus className="size-3" /> 0%
      </span>
    );
  }
  const isPositive = rounded > 0;
  const isGood = invert ? !isPositive : isPositive;
  return (
    <span
      className={`inline-flex items-center gap-0.5 text-xs font-medium ${
        isGood ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"
      }`}
    >
      {isPositive ? <ArrowUp className="size-3" /> : <ArrowDown className="size-3" />}
      {Math.abs(rounded)}%
    </span>
  );
}

function MetricCard({
  label,
  value,
  metric,
  comparisonLabel,
  invert,
}: {
  label: string;
  value: string | number;
  metric: SearchConsoleMetricComparison;
  comparisonLabel: string;
  invert?: boolean;
}) {
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <p className="text-xs text-muted-foreground">{label}</p>
      <p className="mt-1 text-2xl font-semibold">{value}</p>
      <div className="mt-2 flex items-center gap-1.5">
        <ChangeBadge changePercent={metric.changePercent} invert={invert} />
        <span className="text-xs text-muted-foreground truncate">vs {comparisonLabel}</span>
      </div>
    </div>
  );
}

export function SearchConsoleOverviewCards({
  overview,
  comparisonLabel = "periodo precedente",
}: {
  overview: SearchConsoleOverview;
  comparisonLabel?: string;
}) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      <MetricCard
        label="Click"
        value={overview.clicks.current}
        metric={overview.clicks}
        comparisonLabel={comparisonLabel}
      />
      <MetricCard
        label="Impression"
        value={overview.impressions.current}
        metric={overview.impressions}
        comparisonLabel={comparisonLabel}
      />
      <MetricCard
        label="CTR medio"
        value={`${(overview.ctr.current * 100).toFixed(1)}%`}
        metric={overview.ctr}
        comparisonLabel={comparisonLabel}
      />
      <MetricCard
        label="Posizione media"
        value={overview.position.current.toFixed(1)}
        metric={overview.position}
        comparisonLabel={comparisonLabel}
        invert
      />
    </div>
  );
}
