...
export function BlogCardsSection({
  ...
  heading,
  viewAllHref,
  viewAllLabel = "Vedi tutto il blog →",
}: {
  posts: BlogCardPost[];
  heading: string;
  viewAllHref: string;
  ...
  return (
    <section className="px-6 md:px-12 py-20 max-w-7xl mx-auto">
      <div className="flex justify-between items-end mb-12 border-b border-[#222222] pb-10">
        <h2 className="text-xs font-bold tracking-normal uppercase text-[#2e9bd6]">{heading}</h2>
        <Link
          href={viewAllHref}
      ...
      <Link
        href={viewAllHref}
        className="mt-8 block md:hidden text-center text-[#666666] hover:text-white text-xs font-medium uppercase tracking-normal transition-colors"
      >
        {viewAllLabel}
      </Link>
    </section>
